...
function quatToEulerY(x: number, y: number, z: number, w: number): number {
...
export function createServerFish(
  id: string,
  world: RAPIER.World,
...
function applyFacingForce(fish: ServerFish, dt: number): void {
...
function applyRecoveryTorque(fish: ServerFish): void {
  const rot = fish.body.rotation();
  ...
  fish.body.applyTorqueImpulse(
    { x: cX * strength * 0.01, y: 0, z: cZ * strength * 0.01 },
    true
  );
}

function applyVerticalDynamics(fish: ServerFish, dt: number): void {
