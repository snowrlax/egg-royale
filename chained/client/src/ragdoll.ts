...
// Phase B (parked for later — see chained/plan.md): physics drives the actual LowerArmL bone.
// Loop order: animation writes bones → we read them → step physics → overwrite bone with physics result.
// Currently unused; revival is tracked as Phase 5 in the arena-first plan.
export function createRagdoll(physics: Physics, character: Character): Ragdoll {
